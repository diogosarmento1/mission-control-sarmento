...
/* Suporta mais do que 1 utilizador: AUTH_USERS é uma variável de ambiente
 * com uma lista JSON, ex.: [{"username":"diogo","passwordHash":"..."},
 * {"username":"chefe","passwordHash":"..."}]. Para acrescentar alguém no
 * futuro, basta gerar o hash da password dele (gerarHashPassword) e
 * acrescentar um objeto novo a esta lista na Vercel — sem tocar em código. */
function autenticar(usersJsonTexto, username, password) {
  var utilizadores;
  try {
    utilizadores = JSON.parse(usersJsonTexto);
  } catch (err) {
    return false;
  }
  if (!Array.isArray(utilizadores)) return false;
  for (var i = 0; i < utilizadores.length; i++) {
    var u = utilizadores[i];
    if (u && u.username === username && u.passwordHash) {
      return verificarPassword(password, u.passwordHash);
    }
  }
  return false;
}
